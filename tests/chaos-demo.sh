...
show_distribution() {
    ...
    echo -e "  ${DIM}  ┌──────────────────────┬───────────┬────────────┬────────────┐${NC}"
    printf "  ${DIM}  │${NC} ${BOLD}%-20s${NC} ${DIM}│${NC} ${BOLD}%-9s${NC} ${DIM}│${NC} ${BOLD}%-10s${NC} ${DIM}│${NC} ${BOLD}%-10s${NC} ${DIM}│${NC}\n" "Pool" "Count" "Actual %" "Target %"
    echo -e "  ${DIM}  ├──────────────────────┼───────────┼────────────┼────────────┤${NC}"
    ...
    if [[ $scheduled -gt 0 ]]; then
        ...
        printf "  ${DIM}  │${NC} ${GREEN}%-20s${NC} ${DIM}│${NC} ${GREEN}%-9d${NC} ${DIM}│${NC} ${GREEN}%-9s%%${NC} ${DIM}│${NC} ${WHITE}%-10s${NC} ${DIM}│${NC}\n" "🟢 Spot" "$spot" "$spot_pct" "70%"
        printf "  ${DIM}  │${NC} ${BLUE}%-20s${NC} ${DIM}│${NC} ${BLUE}%-9d${NC} ${DIM}│${NC} ${BLUE}%-9s%%${NC} ${DIM}│${NC} ${WHITE}%-10s${NC} ${DIM}│${NC}\n" "🔵 Reserved" "$res" "$res_pct" "30%"
    else
        printf "  ${DIM}  │${NC} %-20s ${DIM}│${NC} %-9d ${DIM}│${NC} %-10s ${DIM}│${NC} %-10s ${DIM}│${NC}\n" "🟢 Spot" "0" "—" "70%"
        printf "  ${DIM}  │${NC} %-20s ${DIM}│${NC} %-9d ${DIM}│${NC} %-10s ${DIM}│${NC} %-10s ${DIM}│${NC}\n" "🔵 Reserved" "0" "—" "30%"
    fi

    if [[ $pending -gt 0 ]]; then
        printf "  ${DIM}  │${NC} ${RED}%-20s${NC} ${DIM}│${NC} ${RED}%-9d${NC} ${DIM}│${NC} ${RED}%-10s${NC} ${DIM}│${NC} ${RED}%-10s${NC} ${DIM}│${NC}\n" "⏳ Pending" "$pending" "—" "0%"
    fi

    echo -e "  ${DIM}  ├──────────────────────┼───────────┼────────────┼────────────┤${NC}"
    printf "  ${DIM}  │${NC} ${BOLD}%-20s${NC} ${DIM}│${NC} ${BOLD}%-9d${NC} ${DIM}│${NC} ${BOLD}%-10s${NC} ${DIM}│${NC} ${BOLD}%-10s${NC} ${DIM}│${NC}\n" "TOTAL" "$total" "100%" "100%"
    echo -e "  ${DIM}  └──────────────────────┴───────────┴────────────┴────────────┘${NC}"
    echo ""
    echo ""
}
...
echo -e "${GREEN}${BOLD}║                                                                              ║${NC}"
echo -e "${GREEN}${BOLD}║  This demo walks through 14 scenarios proving how the Kubernetes scheduler   ║"
echo -e "${GREEN}${BOLD}║  distributes pods across Spot (70%) and Reserved (30%) node pools, and how   ║"
echo -e "${GREEN}${BOLD}║  the system reacts to scaling, failures, and pool removal/restoration.       ║"
echo -e "${GREEN}${BOLD}║                                                                              ║"
echo -e "${GREEN}${BOLD}║  SCENARIOS:                                                                  ║"
echo -e "${GREEN}${BOLD}║    1-4: Scaling & Self-Healing (The 70/30 Trend)                             ║"
echo -e "${GREEN}${BOLD}║    5-8: Node Expansion & Failure (Rack Failures)                             ║"
echo -e "${GREEN}${BOLD}║    9-12: POOL DISRUPTION (Disappearing & Reappearing Nodes)                  ║"
echo -e "${GREEN}${BOLD}║    13-14: Worst-Case & Full Recovery                                         ║"
echo -e "${GREEN}${BOLD}║                                                                              ║"
echo -e "${GREEN}${BOLD}║  ⚠  70/30 is approximate (preferred affinity), not exact.                    ║"
echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
explain "With 13 new pods, the 60/40 weight strategy prevents 
the Spot pool from monopolizing the scheduler. By narrowing 
the scoring gap (60 vs 40), and enforcing a MANDATORY 
topology skew (DoNotSchedule), we force the scheduler to 
fill the Reserved pool to maintain node balance. This 
achieves a distribution much closer to the target 70/30."
section_header "DESCHEDULER VALIDATION"
action "Triggering manual Descheduler run to correct scoring drift..."
if $KUBECTL_CMD get cronjob/descheduler -n kube-system &>/dev/null; then
    $KUBECTL_CMD create job --from=cronjob/descheduler descheduler-manual -n kube-system >/dev/null
    info "Waiting for Descheduler job to complete..."
    $KUBECTL_CMD wait --for=condition=complete job/descheduler-manual -n kube-system --timeout=60s >/dev/null 2>&1 || true
    $KUBECTL_CMD delete job descheduler-manual -n kube-system >/dev/null 2>&1 || true
    wait_for_pods 20
    show_distribution "AFTER DESCHEDULER — Corrected 70/30 Distribution"
    
    explain "The Descheduler identified pods that violated the 70/30
    topology constraints (even if scheduling affinity originally 
    favored Spot). By evicting them, it forced the scheduler to 
    re-evaluate placement, resulting in the desired 70/30 split.
    This is how production environments maintain balance."
else
    note "Descheduler cronjob not found. Skipping manual rebalance step."
fi
...
if ! $KUBECTL_CMD get node -l node-role.kubernetes.io/extra=agent-1 &>/dev/null; then
    $K3D_CMD node create "extra-agent-1" --cluster "$CLUSTER_NAME" > /dev/null 2>&1
fi
wait_for_nodes_ready "$($KUBECTL_CMD get nodes -l '!node-role.kubernetes.io/control-plane,!pool' --no-headers -o name | head -n 1 | cut -d/ -f2)"
action "Labeling the new node as pool=spot, tier/frontend=true..."
# Find the node that was just added (it won't have a pool label yet)
NEW_NODE=$($KUBECTL_CMD get nodes -l '!pool,!node-role.kubernetes.io/control-plane' --no-headers -o name | head -n 1 | cut -d/ -f2)
if [[ -n "$NEW_NODE" ]]; then
    $KUBECTL_CMD label node "$NEW_NODE" pool=spot tier/frontend=true --overwrite
    $KUBECTL_CMD label node "$NEW_NODE" node-role.kubernetes.io/extra=agent-1 --overwrite
fi
...
if ! $KUBECTL_CMD get node -l node-role.kubernetes.io/extra=agent-2 &>/dev/null; then
    $K3D_CMD node create "extra-agent-2" --cluster "$CLUSTER_NAME" > /dev/null 2>&1
fi
if ! $KUBECTL_CMD get node -l node-role.kubernetes.io/extra=agent-3 &>/dev/null; then
    $K3D_CMD node create "extra-agent-3" --cluster "$CLUSTER_NAME" > /dev/null 2>&1
fi
# Label them by finding nodes without a pool label
while read -r node; do
    [[ -z "$node" ]] && continue
    if [[ "$node" == *"agent-2"* ]]; then
        $KUBECTL_CMD label node "$node" pool=spot tier/frontend=true node-role.kubernetes.io/extra=agent-2 --overwrite
    else
        $KUBECTL_CMD label node "$node" pool=reserved tier/frontend=true node-role.kubernetes.io/extra=agent-3 --overwrite
    fi
done < <($KUBECTL_CMD get nodes -l '!pool,!node-role.kubernetes.io/control-plane' --no-headers -o name | cut -d/ -f2)
